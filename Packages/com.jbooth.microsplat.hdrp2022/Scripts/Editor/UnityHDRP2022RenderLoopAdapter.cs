...
{
   public class UnityHDRP2022RenderLoopAdapter : IRenderLoopAdapter
   {
      ...
      {
         return "_MSRENDERLOOP_UNITYHDRP2022";
      }
      ...
      public void Init(string[] paths)
      {
         ...
         {
            string p = paths[i];
            if (p.EndsWith("microsplat_template_hdrp2022_template.txt"))
            {
            ...
            else if (p.EndsWith("microsplat_template_hdrp2022_passforward.txt"))
            {
               templatePassForward = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith("microsplat_template_hdrp2022_passgbuffer.txt"))
            {
               templatePassGBuffer = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith("microsplat_template_hdrp2022_passdepthonly.txt"))
            {
               templatePassDepthOnly = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith("microsplat_template_hdrp2022_passmeta.txt"))
            {
               templatePassMeta = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith("microsplat_template_hdrp2022_passsceneelection.txt"))
            {
               templatePassSceneSelection = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith("microsplat_template_hdrp2022_passshadow.txt"))
            {
               templatePassShadow = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith("microsplat_template_hdrp2022_shared.txt"))
            {
            ...
            else if (p.EndsWith("microsplat_template_hdrp2022_vert.txt"))
            {
               templateVert = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith("microsplat_template_hdrp2022_include.txt"))
            {
               templateInclude = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith("microsplat_template_hdrp2022_passmotionvectors.txt"))
            {
               templatePassMotionVector = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith("microsplat_template_hdrp2022_passpicking.txt"))
            {
               templatePassPicking = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith("microsplat_template_hdrp2022_passtransparentdepthprepass.txt"))
            {
               templateTransparentDepth = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith("microsplat_template_hdrp2022_passfullscreendebug.txt"))
            {
               templatePassFullScreenDebug = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith("microsplat_template_hdrp2022_passforwardunlit.txt"))
            {
      ...
      StringBuilder BuildTemplate(Blocks blocks, string fallbackOverride = null)
      {
         ...
         header.AppendLine("//   Render Pipeline: HDRP2022");
