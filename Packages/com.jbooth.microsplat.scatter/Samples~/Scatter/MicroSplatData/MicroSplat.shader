...
Shader "MicroSplat/Scatter"
{
   Properties
   {
      ...
      _ScatterControl("Scatter Control", 2D) = "black" {}
      _ScatterDiffuse("Scatter Diffuse", 2DArray) = "white" {}
      _ScatterNSAO("Scatter NSAO", 2DArray) = "bump" {}
      _ScatterUVScale("Scatter UV Scale", Vector) = (1, 1, 0, 0)
      _ScatterUVScale2("Scatter UV Scale2", Vector) = (1, 1, 0.5, 0.5)
   ...
   #pragma fragment Frag
      ...
      #define _SCATTER 1
      #define _SPLATTERSECONDLAYER 1
      #define _PERTEXSCATTERBLENDMODE 1
      #define _PERTEXSCATTERALPHABOOST 1
...
#   define UNITY_LOOP
      ...
      float4 _ScatterControl_TexelSize;
      float4 _ScatterDiffuse_TexelSize;
      float2 _ScatterUVScale;
      float4 _ScatterUVScale2;
...
void PrepareStochasticUVs(float scale, float2 uv, out float2 uv1, out float2 uv2, out float2 uv3, out half3 weights)
{
   ...
   weights = half3(w1, w2, w3);
      ...
      half4 ScatterBlendNormal(half4 normSAO, half4 n, half4 a, float mode, float alpha, float alphaMult, float weight)
      {
         ...
         {
            half4 ret = lerp(normSAO, n, weight);
         ...
         int i0, int i1, int i2, half3 weights, float alpha, float2 uvOffset)
      {
         ...
         if (blend0 == 1)
         {
            a0.a = SAMPLE_TEXTURE2D_ARRAY_GRAD(_ScatterDiffuse, my_point_repeat_sampler, uv * scale0 + uvOffset, i0, dx * scale0, dy * scale0).a;
         }
         UNITY_BRANCH
         if (blend1 == 1)
         {
            a1.a = SAMPLE_TEXTURE2D_ARRAY_GRAD(_ScatterDiffuse, my_point_repeat_sampler, uv * scale1 + uvOffset, i1, dx * scale1, dy * scale1).a;
         }
         UNITY_BRANCH
         if (blend2 == 1)
         {
            a2.a = SAMPLE_TEXTURE2D_ARRAY_GRAD(_ScatterDiffuse, my_point_repeat_sampler, uv * scale2 + uvOffset, i2, dx * scale2, dy * scale2).a;
         }
      ...
      void ApplyScatter(Input i, inout half4 albedo, inout half4 normalSAO, inout half3 surfGrad, float2 controlUV, float camDist)
      {

         #if _MEGASPLAT
            MSBRANCHOTHER(i.scatter0.w)
            {
               int i0 = round(i.scatter0.x / max(i.baryWeights.x, 0.00001));
               int i1 = round(i.scatter0.y / max(i.baryWeights.y, 0.00001));
               int i2 = round(i.scatter0.z / max(i.baryWeights.z, 0.00001));
            ...
            {
               int i3 = round(i.scatter1.x / max(i.baryWeights.x, 0.00001));
               int i4 = round(i.scatter1.y / max(i.baryWeights.y, 0.00001));
               int i5 = round(i.scatter1.z / max(i.baryWeights.z, 0.00001));
...
float3 GetTessFactors ()
{
   ...
   Dependency "BaseMapShader" =  "Hidden/MicroSplat/Scatter_Base231034176"
   Fallback "Hidden/MicroSplat/Scatter_Base231034176"
