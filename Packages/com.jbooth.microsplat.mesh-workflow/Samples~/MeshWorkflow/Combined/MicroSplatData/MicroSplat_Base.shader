...
Shader "Hidden/MicroSplat/Combined_Base" {
   Properties {
...
void PrepareStochasticUVs(float scale, float2 uv, out float2 uv1, out float2 uv2, out float2 uv3, out half3 weights)
{
   ...
   weights = half3(w1, w2, w3);
            ...
            void SampleMeshCombined(inout half4 inAlbedo, inout half4 inNormal, inout half4 inEmisMetal, inout half inAlpha, Config config, half4 weights)
            {
