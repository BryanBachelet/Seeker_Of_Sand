...
Shader "Hidden/MicroSplat/VertexPainted_Base" {
   Properties {
      ...
      _MeshAlphaIndex("MeshAlphaIndex", Int) = 0
      _StandardDiffuse("Diffuse/AO", 2D) = "white" { }
      _StandardDiffuseTint("Tint", Color) = (1,1,1,1)
      _StandardNormal("Normal", 2D) = "bump" { }
      _CombinedMeshNormalBlend("Normal Blend", Range(0, 1.5)) = 1
      _StandardSmoothness("Smoothness", Range(0, 1)) = 0
      _StandardMetal("Metallic", Range(0, 1)) = 0
      _StandardUVScaleOffset("UV Scale and Offset", Vector) = (1, 1, 0, 0)
   ...
   SubShader {
      ...
      #define _MICROVERTEXMESH 1
      #define _MESHCOMBINED 1
      ...
      // debug mode shows how many samples branches would save us.
      #if _BRANCHSAMPLES
      ...
      void DoDebugOutput(inout MicroSplatLayer l)
      {
            ...
            ClearAllButAlbedo(l.Smoothness.xxx * saturate(l, l.Albedo.z+1));
         #elif _DEBUG_OUTPUT_METAL
            ClearAllButAlbedo(l.Metallic.xxx * saturate(l, l.Albedo.z+1));
         #elif _DEBUG_OUTPUT_AO
            ClearAllButAlbedo(l.Occlusion.xxx * saturate(l, l.Albedo.z+1));
         #elif _DEBUG_OUTPUT_EMISSION
            ClearAllButAlbedol.Emission * saturate(l, l.Albedo.z+1));
...
void PrepareStochasticUVs(float scale, float2 uv, out float2 uv1, out float2 uv2, out float2 uv3, out half3 weights)
{
   ...
   weights = half3(w1, w2, w3);
      ...
      void EncodeVertex(appdata i, inout Input IN)
      {
         ...
         #if _STREAMS || _LAVA || _PUDDLES || _WETNESS
            IN.s0 = DecodeToFloat4(i.texcoord2.w);
         #endif
      }
      
      void VertexSetup(Input i, out half4 weights, float2 uv, out Config config, float3 worldPos)
      {
         config = (Config)0;
         ...
         config.uv3 = float3(scaledUV, indexes.w);
            ...
            void SampleMeshCombined(inout half4 inAlbedo, inout half4 inNormal, inout half4 inEmisMetal, inout half3 inSpecular,
               inout half inAlpha, inout half SSSThickness, inout half3 SSSTint, Config config, half4 weights)
            {
               ...
               #if _USESPECULARWORKFLOW
                  specular = lerp(specular, inSpecular, inAlpha);
               #endif
               inAlpha = diff.a;
...
UNITY_INSTANCING_BUFFER_END(Terrain)
      ...
      void SampleAlbedo(inout Config config, TriplanarConfig tc, inout RawSamples s, MIPFORMAT mipLevel, half4 weights)
      {
         #if _DISABLESPLATMAPS
         return;
         #endif
