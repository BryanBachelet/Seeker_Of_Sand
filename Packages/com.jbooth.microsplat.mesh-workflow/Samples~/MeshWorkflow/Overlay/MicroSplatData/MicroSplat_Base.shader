...
Shader "Hidden/MicroSplat/Overlay_Base"
{
   Properties
   {
...
{
#if _MICROTERRAIN && !_TESSDISTANCE && defined(UNITY_INSTANCING_ENABLED)
...
{
   #if _TESSDISTANCE
      return float3(_TessData1.x, _TessData2.x, _TessData2.y);
   #endif
   return 0;
}
    ...
    void SurfaceFunction(inout Surface o, inout ShaderData d)
    {
       ...
       float3 worldNormalVertex = d.worldSpaceNormal;
            ...
            #if _STANDARD || _URP   // fuckin hell Unity
               geomNormal.z *= -1;
            #endif
            ...
            #if _STANDARD || _URP
               geomNormal.z *= -1;   // fuckin hell Unity
            #endif
            float3 geomTangent = normalize(cross(geomNormal, float3(0, 0, 1)));
            float3 geomBitangent = normalize(cross(geomTangent, geomNormal));
            worldNormalVertex = geomNormal;
        ...
        // I don't understand why we have to do this, but we do - in both MS and Better shaders
        l.Normal.y *= -1;

         #if (_PERPIXNORMAL || defined(UNITY_INSTANCING_ENABLED)) && (_MICROTERRAIN || _MICROMESHTERRAIN) && !PASSSHADOW     
