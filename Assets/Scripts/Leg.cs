...
public class Leg : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Physics.Raycast(rayOrigin.position, bodyTransform.up.normalized * -1, out hit, maxRayDist, groundLayer))
        {
        ...
        tipDistancedebug = TipDistance;
        tipMoveDistDebug = tipMoveDist;
        animatingDebug = Animating;
        moveableDebug = Movable;
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawSphere(TipPos, 1f);

        Gizmos.color = Color.red;
        Gizmos.DrawLine(TipPos, RaycastTipPos);

        Gizmos.color = Color.yellow;
        Gizmos.DrawSphere(ikTarget.transform.position, 1f);
    }
}
