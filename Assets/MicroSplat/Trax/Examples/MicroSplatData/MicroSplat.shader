...
Shader "MicroSplat/TraxExample" {
   Properties {
      ...
      _TraxUVScales("UV scale", Vector) = (1,1,0,0)
      _TraxTextureBlend("Texture Blend", Range(0, 1)) = 1
   ...
   SubShader {
     ...
     #pragma instancing_options assumeuniformscaling nomatrices nolightprobe nolightmap forwardadd
      ...
      struct Input 
      {
         ...
         #if (_MICRODIGGERMESH || _MICROVERTEXMESH) && !_MEGASPLAT
            fixed4 w0;
            #if !_MAX4TEXTURES
               fixed4 w1;
            #endif
         ...
         #if _MEGASPLAT
             float4 layer0; // RGB*index, blend weight
             float4 layer1;
             #if _SCATTER
                float4 scatter0; // scatter, blend weight
             #endif
             #if _SPLATTERSECONDLAYER
                float4 scatter1; // scatter, blend weight
             #endif
      ...
      void ConstructSurfaceGradientTBN(Input i)
      {
         ...
         float renormFactor = 1.0 / length(surfNormal.xyz);
...
void PrepareStochasticUVs(float scale, float2 uv, out float2 uv1, out float2 uv2, out float2 uv3, out half3 weights)
{
   ...
   weights = half3(w1, w2, w3);
         ...
         void ApplyTrax(inout RawSamples samples, Config config, float3 worldPos, float traxBuffer, float3 traxNormal)
         {
            ...
            half4 normSAO = MICROSPLAT_SAMPLE_TEX2D_SAMPLER_GRAD(_TraxNSAO, _Diffuse, uv, fsdx, fsdy).garb;
            ...
            half4 normSAO0 = MICROSPLAT_SAMPLE_TEX2DARRAY_GRAD(_TraxArrayNSAO, float3(uv, config.uv0.z), fsdx, fsdy).garb;
            ...
            half4 normSAO1 = MICROSPLAT_SAMPLE_TEX2DARRAY_GRAD(_TraxArrayNSAO, float3(uv, config.uv1.z), fsdx, fsdy).garb;
            ...
            h1 = lerp(traxBuffer1, h1, _TraxInterpContrast);
               ...
               half4 normSAO2 = MICROSPLAT_SAMPLE_TEX2DARRAY_GRAD(_TraxArrayNSAO, float3(uv, config.uv2.z), fsdx, fsdy).garb;
               ...
               half4 normSAO3 = MICROSPLAT_SAMPLE_TEX2DARRAY_GRAD(_TraxArrayNSAO, float3(uv, config.uv3.z), fsdx, fsdy).garb;
...
UNITY_INSTANCING_BUFFER_END(Terrain)
      ...
      MicroSplatLayer Sample(Input i, half4 weights, inout Config config, float camDist, float3 worldNormalVertex, DecalOutput decalOutput)
      {
         ...
         #if _SURFACENORMALS
            o.Normal.xyz = ResolveNormalFromSurfaceGradient(surfGrad);
            o.Normal = mul(GetTBN(i), o.Normal.xyz);
         #endif
...
ENDCG
   ...
   Dependency "BaseMapShader" = "Hidden/MicroSplat/TraxExample_Base-826634602"
